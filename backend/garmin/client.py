...
class GarminAlpha200Client:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Autentisert som {self.email}")
            return True
        except GarminConnectAuthenticationError as e:
            logger.error(f"Autentiseringsfeil: {e}")
            self._authenticated = False
            return False
        except GarminConnectConnectionError as e:
            logger.error(f"Tilkoblingsfeil: {e}")
            self._authenticated = False
            return False
        except Exception as e:
            logger.error(f"Ukjent feil ved autentisering: {e}")
            self._authenticated = False
            return False
